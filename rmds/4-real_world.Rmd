...
#                                        matX = df_data[, tb_fits$miRNAs[i_fit],
#                                                       drop = FALSE] %>%
#                                          as.matrix(),
...
#                                                  drop = FALSE] %>%
#                                     as.matrix(),
#                                   matZ = df_data[, tb_fits$mRNAs[i_fit],
#                                                  drop = FALSE] %>% 
#                                     as.matrix(),
#                                   study = df_data$study,
#                                   missing = df_data$missing,
...
tb_summary <- tb_fits %>% 
  ...
  dplyr::mutate(
    coef_total = results[[1]]$fit_marginal$coef,
    var_total = results[[1]]$fit_marginal$Sigma[1, 1],
    coef_direct = results[[1]]$fit_opt$coef[1],
    var_direct = results[[1]]$fit_opt$Sigma[1, 1],
    coef_indirect = results[[1]]$fit_opt$coef_indirect,
    var_indirect = results[[1]]$fit_opt$Sigma_indirect[1, 1],
    coef_conditional = results[[1]]$fit_opt$coef[2],
    var_conditional = results[[1]]$fit_opt$Sigma[2, 2],
    coef_alpha = results[[1]]$fit_opt$coef_alpha,
    var_alpha = results[[1]]$fit_opt$Sigma_alpha[1, 1]) %>% 
  dplyr::ungroup() %>% 
  dplyr::select(-`1:dplyr::n()`, -results)

tb_coef <- tb_summary %>% 
  dplyr::select(miRNA, mRNA, dplyr::starts_with("coef")) %>% 
  ...
  dplyr::mutate(effect = effect %>% 
                  stringr::str_replace_all(stringr::fixed("coef_"),
                                           "")) %>% 
  dplyr::filter(effect != "total" | !duplicated(miRNA))
tb_var <- tb_summary %>% 
  dplyr::select(miRNA, mRNA, dplyr::starts_with("var")) %>% 
  ...
  dplyr::filter(effect != "total" | !duplicated(miRNA))
tb_plot <- tb_coef %>% 
  dplyr::left_join(tb_var, by = c("miRNA", "mRNA", "effect")) %>% 
...
tb_plot <- tb_plot %>% 
  dplyr::mutate(miRNA_panel = factor(miRNA, 
                ...
                p_anno = dplyr::case_when(p < 0.001 ~ "***",
                                          p < 0.01 ~ "**",
                                          p < 0.05 ~ "*",
                                          TRUE ~ ""))
...
p_fig <- tb_plot %>% 
  ...
  scale_y_continuous(breaks = seq(from = -1.5, to = 2, by = 0.5)) +
