...
tb_fits <- tidyr::crossing(miRNAs = miRNAs, 
                           mRNAs = mRNAs)
# l_results <- 
...
tb_print <- tb_fits %>% 
  ...
  dplyr::mutate(coef_indirect = results[[1]]$coef_indirect,
                sd_indirect = sqrt(results[[1]]$Sigma_indirect),
                beta_Z = results[[1]]$coef[2],
                sd_beta = sqrt(results[[1]]$Sigma[2, 2]),
                alpha = results[[1]]$coef_alpha,
                sd_alpha = sqrt(results[[1]]$Sigma_alpha[1, 1])) %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(p_indirect = 2 * (1 - pnorm(abs(coef_indirect / sd_indirect)))) %>% 
  dplyr::arrange(p_indirect) %>% 
  dplyr::select(miRNAs, mRNAs, coef_indirect, sd_indirect, p_indirect,
                beta_Z, sd_beta, alpha, sd_alpha)
tb_print %>% 
