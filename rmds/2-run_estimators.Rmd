...
load(paste0(dir_project, "results/1-simulate_datasets/scenarioIV.RData"))
tb_sim <- tb_sim_IV %>% dplyr::mutate(scenario = 4)
```
```{r run estimators}
B <- 500
N <- nrow(tb_sim)
start.time <- Sys.time()
l_results_IV <-  Rmpi::mpi.parLapply(
  seq_len(N),
  function(i,
           dir_output,
           tb_sim,
...
save(l_results_IV, file = paste0(dir_output, "fits_updated_IV.RData"))
```